...
def test_dd_tightness(data_set, n_j, n_m):
    ...
    while tightness <= 2.0:
        ...
        for data in data_set:
            ...
            job_deadlines = generate_deadlines(times=env.dur, lead_times=env.lt, deadline_tightness=tightness)
            ...
            job_finish_times = env.finish_time[np.arange(env.finish_time.shape[0]),
                                               (env.finish_time != 0).cumsum(1).argmax(1)].astype(np.int32)
...
def generate_deadline_data(data_set, c, n_j, n_m):
    ...
    for data in data_set:
        _ = env.reset(data)
        job_deadlines = generate_deadlines(times=env.dur, lead_times=env.lt, deadline_tightness=c)
...
def baseline_performance(data_set, deadline_data_set, n_j, n_m):
    ...
    for rule in rule_set:
        ...
        avg_results[(rule, 'c_max')] = round(sum(results_per_env[(rule, 'c_max')]) / len(results_per_env[(rule, 'c_max')]), 2)
        avg_results[(rule, 't_frac')] = round(sum(results_per_env[(rule, 't_frac')]) / len(results_per_env[(rule, 't_frac')]), 2)
        avg_results[(rule, 't_max')] = round(sum(results_per_env[(rule, 't_max')]) / len(results_per_env[(rule, 't_max')]), 2)
        avg_results[(rule, 't_tot')] = round(sum(results_per_env[(rule, 't_tot')]) / len(results_per_env[(rule, 't_tot')]), 2)
